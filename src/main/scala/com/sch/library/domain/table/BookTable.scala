...
import com.sch.library.domain.{BookStatus, Book}
import slick.driver.PostgresDriver.api._ //TODO how to remove dependency?
...
final class BookTable(tag: Tag) extends Table[Book](tag, "book") {
  ...
  override def * : ProvenShape[Book] = (id.?, title, authors, inventoryNumber.?, status.?) <> ((Book.apply  _).tupled, Book.unapply)
}
